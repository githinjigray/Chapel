codeunit 50018 "Bank Reconciliation Approval"
{
    ...
    var
        ...
        EmpReqApprWorkflowCodeTxt: Label 'BankRec';
    ...
    var
        BankReconciliation: Record "Imprest Header";
    ...
    begin
        ...
        WorkflowSetup.InsertDocApprovalWorkflowSteps(Workflow,
          BuildBankReconciliationConditions(BankReconciliation.Status::Open),
          RunWorkflowOnSendBankReconciliationForApprovalCode(),
          BuildBankReconciliationConditions(BankReconciliation.Status::"Pending Approval"),
          RunWorkflowOnCancelBankReconciliationApprovalRequestCode(),
          WorkflowStepArgument, true);
    end;

    procedure BuildBankReconciliationConditions(Status: Option Open,"Pending Approval",Approved): Text
    var
        BankReconciliation: Record "Imprest Header";
        BankReconciliationTypeCondnTxt: Label '<?xml version="1.0" encoding="utf-8" standalone="yes"?><ReportParameters><DataItems><DataItem name="Imprest Header">%1</DataItem><DataItem name="Imprest Line">%2</DataItem></DataItems></ReportParameters>';
    begin
        BankReconciliation.SetRange(Status, Status);
        exit(StrSubstNo(BankReconciliationTypeCondnTxt, WorkflowSetup.Encode(BankReconciliation.GetView(false))));
    end;
